...
= 3.3.0 =
The product page calculator switch now decides whether a shipping zone answers, not only whether the field is drawn. If you use Central do Frete in more than one zone and turned the calculator off in one of them, shoppers with a postcode in that zone now read that the calculation is not available there, instead of getting that zone's prices. The **Ativar método de entrega** checkbox now governs the product page calculator too, so a zone you switched the method off in stops quoting there as well. With both on everywhere, which is the default, nothing changes. Read this if you define any shipping zone by state: the product page calculator now works the state out from the postcode, so it matches the same shipping zone the cart and the checkout match, where it used to fall through to a broader zone and answer with that zone's token, fee and rules. Expect quotes on your product pages to change in a store where you touched nothing - a state zone above a country wide one now answers for the postcodes in it, which is the zone you set up. Read this too whether you use one zone or several: the product page now answers only for a postcode that falls inside a zone carrying Central do Frete. With the method in a single zone the calculator used to skip the zone match and price every postcode from that zone, so if it covers one state or a range of postcodes rather than the whole country, shoppers outside it now read that the delivery area could not be identified - the same thing the cart has always told them. A postcode no published range covers leaves the state blank and falls back to the behaviour without one: with a zone of yours that carries Central do Frete defined by state, it gets no price and says the delivery area could not be identified. A store postcode that is filled in but is not 8 numbers now quotes from your account's pickup address instead of failing, and the settings screen says the postcode is invalid. This also corrects what the 3.2.0 notice below says about zones defined by state: the calculator no longer answers that it does not serve the postcode.
