...
* A postcode no published range covers leaves the state blank rather than guessed, and falls back to what this release already does without one: if you define any shipping zone by state, the product page shows no price and says the delivery area could not be identified, instead of answering from a zone that may only look like a match. One range sits there today - 78900000-78999999, listed for Rondônia before the renumbering to 768xx, where nothing is allocated. A guessed state would be a wrong zone quoting with nothing on screen to say so, which is worse than no price
